...
:loop_(loop),channel_(new Channel(loop,connfd)),fd_(connfd),error_(false),
connectionState_(H_CONNECTED),method_(METHOD_GET),HTTPVersion_(HTTP_11),
state_(STATE_PARSE_URI),hState_(H_START),nowReadPos_(0),keepAlive_(false)
{
...
void HttpData::handleError(int fd, int err_num, string short_msg)
{
    ...
    body_buff += "<hr><em> LJ's Web Server</em>\n</body></html>";

    header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
    header_buff += "Content-Type: text/html\r\n";
    header_buff += "Connection: Close\r\n";
    header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
    header_buff += "Server: LJ's Web Server\r\n";;
    header_buff += "\r\n";
    // 错误处理不考虑writen不完的情况
